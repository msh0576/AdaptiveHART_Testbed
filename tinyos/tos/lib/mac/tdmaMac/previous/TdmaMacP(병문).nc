...
//	flownum , currentid, beforeID, nextID, slot
		...
		if(TOS_NODE_ID == 5)
		{
			...
			data_scheduling_matrix[0][2] = 99; // 전에
		...
		else if (TOS_NODE_ID == 7)
		{
			...
			data_scheduling_matrix[0][2] = 6;
			data_scheduling_matrix[0][3] = 8;
			data_scheduling_matrix[0][4] = 6;
		}
		else if (TOS_NODE_ID == 8)
		{
			data_scheduling_matrix[0][0] = -1;
			data_scheduling_matrix[0][1] = -1;
			data_scheduling_matrix[0][2] = -1;
			data_scheduling_matrix[0][3] = -1;
			data_scheduling_matrix[0][4] = -1;
		}
  ...
  event void Slotter.slot(uint8_t slot) {
		...
		atomic currentSlot = slot;
			...
			//	dataPacket = forwardDataPacket;
				...
				//if(data_scheduling_matrix[0][0] == dataPacket->flowId) {
		...
		else if (slot == 14) // Last slot of the superframe
		{
			/** @brief Check if radio is already off */
			if(radioOff)
			{
   ...
   if(call ACK.wasAcked(msg));
	...
	//			}
			...
			printf("curSlot = %u, data  : %u,  \n", currentSlot ,(uint64_t)dataPacketTemp->data);
		//	printf("", (uint64_t)dataPacketTemp->data);
		//	printf("flowId: %d \n", (uint8_t)dataPacketTemp->flowId);
		//	printf("%d",currentSlot);
