...
implementation {
	...
	components new QueueC(notification_t *, 2) as notifyQueueC;
	components PrintfC;
	...
	/** @brief Time Sync stuff */
	  components CC2420PacketC;
		components TimeSync32kC as BackGroundSync;
		TdmaMacP.GlobalTime -> BackGroundSync;
		TdmaMacP.TimeSyncInfo -> BackGroundSync;
	  TdmaMacP.PacketTimeStamp -> CC2420PacketC;

	//	TdmaMacP.phySyncSend -> Phy.AMSend[AM_DMAMAC_SYNC]; // 11 is the id used for notification packets
	//	TdmaMacP.phySyncReceive -> Phy.Receive[AM_DMAMAC_SYNC];
	 // TdmaMacP.TimeSyncPacket -> Phy;
