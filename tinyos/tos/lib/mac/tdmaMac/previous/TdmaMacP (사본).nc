...
//	flownum , currentid, beforeID, nextID, slot
		...
		if(TOS_NODE_ID == 5)
		{
			...
			data_scheduling_matrix[0][4] = 4; // 전송 슬롯

			data_scheduling_matrix[1][0] = 2;
			data_scheduling_matrix[1][1] = 5;
			data_scheduling_matrix[1][2] = 6;
			data_scheduling_matrix[1][3] = 2;
			data_scheduling_matrix[1][4] = 10;
		}
		else if (TOS_NODE_ID == 6)
		{
			...
			data_scheduling_matrix[0][4] = 5;

			data_scheduling_matrix[1][0] = 2;
			data_scheduling_matrix[1][1] = 6;
			data_scheduling_matrix[1][2] = 3;
			data_scheduling_matrix[1][3] = 5;
			data_scheduling_matrix[1][4] = 9;
		}
  ...
  event void Slotter.slot(uint8_t slot) {
		...
		//printf("S: %u, SFL %d \n",currentSlot,call FrameConfiguration.getFrameLength());
			if(slot == 0)
			{
				gap = call GlobalTime.getLocalTime() - myGlobalTime;
		//		printf("    superframe time : %d\n",gap);
		...
		else if (data_scheduling_matrix[flow_idx][2] == 99 && data_scheduling_matrix[flow_idx][4] - 1 ==slot)
		{
			sequenceCount++;
		...
		//			printf("Waking up sleeing radio \n");
				...
				if(call phyDataSend.send(data_scheduling_matrix[flow_idx][3], &selfDataPkt, sizeof(dmamac_data_t)) == SUCCESS)
							{
								atomic phyLock = TRUE;
		...
		else if  (!call forwardQueue.empty())
		{
			...
			for(flow_idx = 0; flow_idx<2; flow_idx++){
				if(dataPacket->flowId - 1 == flow_idx){
				 if(data_scheduling_matrix[flow_idx][4] ==slot){
				 		//printf("Check!\r\n");
					 ...
					 //		printf("Waking up sleeing radio \n");
						 ...
						 if(call phyDataSend.send(data_scheduling_matrix[dataPacket->flowId -1][3], &selfDataPkt, sizeof(dmamac_data_t)) == SUCCESS)
							 {
									 atomic phyLock = TRUE;
   ...
   if(call ACK.wasAcked(msg))
	...
	event message_t * phyDataReceive.receive(message_t *msg, void *payload, uint8_t len){
		...
		printf("<R>: Source: %d,", (uint16_t)dataPacketTemp->nodeId);
	  printf("data  : %u,", (uint64_t)dataPacketTemp->data);
		printf("flowId: %d \n", (uint8_t)dataPacketTemp->flowId);
	...
	//	printf("<N> Power : %u, RSSI : %d dBm \n",outputPower,(rssi-RSSI_OFFSET));
		...
		if(!(TOS_NODE_ID  == 3 && realSrc  == 0) && !(TOS_NODE_ID  == 0 && realSrc  == 3))
		{
			if(dataBuffer != NULL)
		 	{
			...
			//if(realSrc ==13)
		//	nbReceivedPkts[0][src]++;
		//else
		//	nbReceivedPkts[1][src]++;
